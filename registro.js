...
function renderParticipantDashboard(pid){
  ...
  const bodyHtml = DASH_TAB==='perfil' ? buildDashPerfilHtml(p)
    ...
    : DASH_TAB==='elim' ? buildDashElimHtml(p)
    : DASH_TAB==='avanzado' ? buildDashAvanzadoHtml(p)
...
function buildDashElimHtml(p){
  ...
  let llaveOk=0, llaveTot=0, totalPts=0, partJugados=0;
  ...
  const llavePtsTotal = Array.from({length:32},(_,i)=>i+73).filter(pid=>isLlaveCorrecta(name,pid)).length*2;
  ...
  let html = `<div class="brkt-summary">
    ...
    <div class="bsum-item"><div class="bsum-val">${llaveOk}/${llaveTot}</div><div class="bsum-lbl">llaves ✓</div></div>
