...
function importarCorreosClaves(file){
  ...
  reader.onload = (e)=>{
    try{
      ...
      // Acepta tanto el backup automático de runMigracionPrivacidad()
      // ({tipo, fecha, dbAntesDeLaMigracion:{participants:[...]}}) como
      // un volcado de DB completo con participants en la raíz (ej. el
      // que descarga "Exportar JSON" en otra parte del panel).
      ...
      if(!participantsBackup || !Array.isArray(participantsBackup)){
        throw new Error("No encontré una lista de participantes en este archivo.");
      }

      // id -> {clave, email} de lo que trae el backup. Se matchea por
      // id (no por nombre/correo) porque es el único campo que no
      // cambia nunca para un mismo participante, esté como esté el
      // resto de sus datos.
      const porId = {};
      ...
      const idsConDatos = Object.keys(porId);
      if(!idsConDatos.length){
        throw new Error("El archivo no tiene clave ni correo guardado para ningún participante.");
      }

      let coincidencias = 0;
      DB.participants.forEach(p=>{ if(porId[p.id]) coincidencias++; });
      if(!coincidencias){
        throw new Error("Ningún participante actual coincide con los ids de este archivo -- ¿es el backup correcto?");
      }

      if(!confirm(`Encontré clave/correo guardado para ${coincidencias} de los ${DB.participants.length} participantes actuales en este archivo. Esto va a reemplazar su clave/correo actual por el del backup y los va a volver a guardar en el documento privado. Antes de aplicar nada se descarga un backup del estado actual, por si esto tampoco era lo que hacía falta. ¿Continuar?`)) return;
      ...
      saveData(DB);
      render();
      toast(`✓ Se importaron clave/correo para ${aplicados} participante(s). Re-sincronizando al documento privado.`);
    }catch(err){
...
function renderAdmin(){
  ...
  c.innerHTML = `
    ...
    <div class="card" id="migracion_privacidad_card">
      <div class="card-title">🔐 Migración de privacidad (clave/correo)</div>
      <div class="note">Hasta ahora, la clave de 6 dígitos y el correo de cada participante viajaban dentro del mismo documento público que el Ranking necesita leer — cualquiera que abriera la página podía leerlos. Este botón mueve clave y correo de TODOS los participantes a un documento separado (solo el propio dueño o el admin pueden leerlo) y reescribe el documento público sin esos dos campos. No toca predicciones, nombre, ciudad, ni ningún otro dato — y es seguro correrlo más de una vez (a quien ya esté migrado, no le cambia nada). Descarga un backup automático antes de escribir nada. <b>Hace falta correrlo una sola vez</b> para que el arreglo tenga efecto sobre los participantes que ya existen (los nuevos que se registren de ahora en más ya quedan bien automáticamente).</div>
      <button class="rg-btn rg-btn-primary" id="a_migrar_privacidad">🔐 Migrar clave/correo a privado (27 participantes)</button>
    </div>
    ...
    <div class="card">
      ...
      <div class="rg-btn-row" style="margin-top:.75rem">
        ...
        <button class="rg-btn rg-btn-ghost" id="a_import_claves" title="Restaura correo/clave desde un backup JSON (ej. el que se descarga solo al migrar o al importar)">⬆️ Importar correos y claves</button>
        <input id="import_claves_file" type="file" accept=".json" style="display:none">
  ...
  document.getElementById('a_migrar_privacidad').addEventListener('click', ()=>{
    if(!confirm(`Esto va a mover clave y correo de ${(DB.participants||[]).length} participante(s) a un documento privado, y reescribir el documento público sin esos dos campos. Antes de escribir nada se descarga un backup. ¿Continuar?`)) return;
    runMigracionPrivacidad();
  });
