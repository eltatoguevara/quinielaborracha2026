...
// y más adelante el PDF) — nunca se leen desde preds.special para evitar
// que quede un valor manual "viejo" guardado de antes de este cambio.
...
function computeBracket(preds){
  ...
  for(let i=0;i<16;i++){
    const slot = `r32_${i+1}`;
    const a = seeded[i], b = seeded[31-i];
    r32.push({ slot, a, b, winner: koWinner(preds[slot], a, b) });
  }
  const r16 = [];
  for(let i=0;i<8;i++){
    const slot = `r16_${i+1}`;
    ...
    r16.push({ slot, a, b, from:[m1.slot,m2.slot], winner: (a&&b)?koWinner(preds[slot],a,b):null });
  }
  const qf = [];
  for(let i=0;i<4;i++){
    const slot = `qf_${i+1}`;
    ...
    const a=m1.winner, b=m2.winner;
    qf.push({ slot, a, b, from:[m1.slot,m2.slot], winner: (a&&b)?koWinner(preds[slot],a,b):null });
  }
  const sf = [];
  for(let i=0;i<2;i++){
    const slot = `sf_${i+1}`;
    ...
    const a=m1.winner, b=m2.winner;
    sf.push({ slot, a, b, from:[m1.slot,m2.slot], winner: (a&&b)?koWinner(preds[slot],a,b):null });
  }
  const thirdA = (sf[0].a && sf[0].b) ? koLoser(preds[sf[0].slot], sf[0].a, sf[0].b) : null;
  const thirdB = (sf[1].a && sf[1].b) ? koLoser(preds[sf[1].slot], sf[1].a, sf[1].b) : null;
  const third = { slot:'third', a:thirdA, b:thirdB, winner: (thirdA&&thirdB)?koWinner(preds['third'],thirdA,thirdB):null };
  const finalA = sf[0].winner, finalB = sf[1].winner;
  const final = { slot:'final', a:finalA, b:finalB, winner: (finalA&&finalB)?koWinner(preds['final'],finalA,finalB):null };
...
/* ════════════════════════════════════════
   ...
   ════════════════════════════════════════ */
const STORE_KEY = "qbRegistroV3"; // mismo formato de siempre; ahora además respaldado en Firestore (doc registro/estado)
const CODE_YEAR = 2026;
const RG_DEFAULT_CONFIG = {
  modoConsultaHabilitado:true, registroAbierto:true,
  loginPorNombreHabilitado:true, // v1.0 — puente temporal para migrar participantes históricos sin correo
  fechaCierre:'', horaCierre:'23:59' // v1.0 — cierre automático global (vacío = sin cierre configurado)
};

function rgEmptyDB(){
  return {participants:[], predictions:{}, nextSeq:1, configGlobal:{...RG_DEFAULT_CONFIG}};
}

// v6.0 (Fase 1) — loadData() ahora solo lee la CACHÉ local (instantáneo, igual
// que antes). El estado real y compartido entre dispositivos llega por
// Firestore vía rgWireFirestoreSync() unos instantes después de cargar la
// página — el mismo patrón que ya usa app.js para quiniela/estado.
function loadData(){
  try{
    ...
    d.configGlobal = {...RG_DEFAULT_CONFIG, ...(d.configGlobal||{})}; // aditivo: respeta lo guardado, completa lo que falte
    return d;
...
// ══════════════════════════════════════════════════════════════
// SINCRONIZACIÓN EN VIVO — Firestore (v6.0, Fase 1)
// Doc único registro/estado (mismo enfoque que quiniela/estado en app.js).
// A diferencia de pushStateToFirestore() en app.js, aquí SÍ escriben tanto
// el admin como cualquier participante (cada uno guarda su propia
// quiniela) — por eso este push NO está condicionado a isAdmin().
// Requiere que las reglas de Firestore permitan escritura pública en
// registro/estado (ver nota de seguridad al final de este archivo).
// ══════════════════════════════════════════════════════════════
...
function rgPushToFirestore(d){
  ...
  fb.setDoc(fb.REGISTRO_DOC, {json: JSON.stringify(d), updatedAt: fb.serverTimestamp()})
    .catch(err=>{
      ...
      console.error("Error al sincronizar registro con Firebase:", err);
      if(err && err.code === 'permission-denied'){
        toast('⚠️ Firestore aún no permite escritura pública en /registro — tu quiniela quedó guardada solo en este dispositivo. Avisa al admin.', true);
      }
    });
}
...
function rgApplyRemote(d){
  ...
  // Si hay un borrador activo en memoria, lo resincronizamos con lo recién llegado
  if(DRAFT_PID){
...
function rgWireFirestoreSync(){
  ...
  fb.onSnapshot(fb.REGISTRO_DOC, (snap)=>{
    ...
    catch(e){ console.error("Error al aplicar registro remoto:", e); }
  }, (err)=>{ console.error("Error de sincronización Firestore (registro):", err); });
}
...
// Código único QB-2026-0001. Usa un contador propio (DB.nextSeq) que nunca
// se reutiliza, ni siquiera si se elimina un participante, para que dos
// personas nunca terminen con el mismo código.
function nextCode(){
...
// v0.8 (Fase 5) — Clave aleatoria de 6 dígitos para las herramientas
// administrativas de generación (individual y masiva). No se valida
// unicidad: la Clave siempre se usa junto al correo para entrar, igual
// que en el registro normal, así que dos claves repetidas no son un
// problema de seguridad real en este prototipo.
function genClave(){
...
function renderInicio(){
  const c = document.getElementById('rg-content');
  if(DRAFT_PID){ renderQuinielaForm(DRAFT_PID, 'inicio'); return; }
  ...
  c.innerHTML = `
    <div class="card center" style="padding:1.75rem 1rem">
      <div style="font-size:38px;margin-bottom:.5rem">⚔️</div>
      <div class="card-title" style="justify-content:center">¿Ya tienes una quiniela registrada?</div>
      <div class="muted" style="margin-bottom:1.1rem;font-size:13px">Elige una opción para continuar.</div>
...
function buildCountryCityFieldsHtml(idPrefix, country, countryIso, city){
  ...
  return `
    ...
    <div class="field-hint" style="margin-top:-6px">Queremos saber desde dónde juegas.</div>`;
}
...
function buildReviewSummaryHtml(p, preds, bracket){
  ...
  const groups = {};
  GROUP_MATCHES.forEach(m=>{ (groups[m.g]=groups[m.g]||[]).push(m); });
  html += `<div class="card"><div class="card-title">⚽ Fase de grupos</div>`;
  Object.keys(groups).sort().forEach(g=>{
    html += `<div class="group-head">Grupo ${g}</div>`;
    groups[g].forEach(m=>{
      const v = preds[m.id]||{};
      const h = (v.h===0||v.h>0)?v.h:'—', a = (v.a===0||v.a>0)?v.a:'—';
      html += `<div class="status-row"><span>${flagOf(m.a)} ${esc(m.a)} vs ${esc(m.b)} ${flagOf(m.b)}</span><span class="ro-score">${h} : ${a}</span></div>`;
    });
  });
  html += `</div>`;

  html += `<div class="card"><div class="card-title">🏆 Fase eliminatoria</div>`;
  if(!bracket.ready){
    html += `<div class="note">Pendiente — faltan partidos de grupos.</div>`;
  }else{
    KO_PHASES.forEach(ph=>{
      const slots = koSlotsOf(bracket, ph.key);
      html += `<div class="group-head">${esc(ph.label)}</div>`;
      slots.forEach(m=>{
        if(!m.a || !m.b){ html += `<div class="status-row"><span class="muted">Pendiente</span></div>`; return; }
        const raw = preds[m.slot];
        const v = (raw && raw._a===m.a && raw._b===m.b) ? raw : null;
        const h = v && Number.isInteger(v.h) ? v.h : '—', a = v && Number.isInteger(v.a) ? v.a : '—';
        html += `<div class="status-row"><span>${flagOf(m.a)} ${esc(m.a)} vs ${esc(m.b)} ${flagOf(m.b)}</span><span class="ro-score">${h} : ${a}</span></div>`;
      });
    });
  }
  ...
  html += `<div class="card"><div class="card-title">✨ Preguntas especiales</div>`;
  const sp = preds.special || {};
  const autoSp = computeAutoSpecial(bracket);
  SPECIAL_QUESTIONS.forEach(q=>{
    const isAuto = AUTO_SPECIAL_IDS.includes(q.id);
    const raw = isAuto ? autoSp[q.id] : sp[q.id];
    const val = (raw!==undefined && raw!=='') ? raw : '—';
    const tag = isAuto ? ` <span class="badge badge-muted" style="margin-left:4px">auto</span>` : '';
    html += `<div class="status-row"><span>${esc(q.label)}${tag}</span><span class="ro-text">${esc(String(val))}</span></div>`;
  });
  html += `</div>`;
  return html;
}
...
function renderQuinielaForm(pid, originTab){
  ...
  const midBtn   = !readOnly ? `<button class="rg-btn rg-btn-ghost" id="wiz_save_exit" style="font-size:11.5px;padding:7px 14px;white-space:nowrap">💾 Guardar y continuar después</button>` : `<span></span>`;
  ...
  document.getElementById('wiz_save_exit')?.addEventListener('click', ()=>{
    flushAutosave();
    toast('Guardado. Puedes continuar después desde "Ver mi quiniela" con tu correo y tu Clave.');
    clearDraft();
    switchToInicioTab();
  });
  ...
  document.getElementById('admin_table_wrap').querySelectorAll('[data-act]').forEach(el=>{
    el.addEventListener('click', ()=>{
      ...
      if(el.dataset.act==='edit'){
        ...
        if(!confirm(`¿Eliminar a "${p.name}" y toda su quiniela?`)) return;
...
function renderAdmin(){
  ...
  document.getElementById('a_reset').addEventListener('click', ()=>{
    ...
    if(!confirm('⚠️ Esto borra TODOS los participantes y predicciones de Mi Quiniela para TODOS (no solo en este navegador — se sincroniza por Firestore). ¿Continuar?')) return;
    if(!confirm('Última confirmación: se perderán todas las quinielas registradas hasta ahora. ¿Seguro?')) return;
    DB = {participants:[], predictions:{}, nextSeq:1, configGlobal:{modoConsultaHabilitado:true, registroAbierto:true, loginPorNombreHabilitado:true, fechaCierre:'', horaCierre:'23:59'}};
    ADMIN_SEARCH = '';
    ADMIN_FILTER = 'all';
    saveData(DB);
...
/* ════════════════════════════════════════
   ...
   queda corrido, es un ajuste de una sola línea en OFICIO_MM más abajo).
   ════════════════════════════════════════ */
const OFICIO_MM = { w: 330, h: 216 }; // Oficio horizontal (~330x216mm / 13x8.5in)
...
function generarPDF(p){
  // v1.0 (mejora 5/6) — "Abrir el PDF en una pestaña nueva" choca con los
  // bloqueadores de pop-ups si la ventana se abre DESPUÉS de esperar a
  // html2canvas (el navegador ya no lo reconoce como gesto del usuario).
  // Por eso la ventana se abre AQUÍ, de forma síncrona, en blanco, y más
  // abajo solo se le cambia la ubicación una vez que el PDF está listo.
  const pdfWindow = window.open('', '_blank');
  if(pdfWindow && pdfWindow.document){
    pdfWindow.document.write('<title>Generando PDF…</title><body style="font-family:sans-serif;padding:48px;text-align:center;color:#666">Generando tu PDF, un momento…</body>');
  }
  ...
  waitForImages(poster).then(()=>{
    ...
    // Página fijada a Oficio horizontal real (no al tamaño del canvas): si el
    ...
    const pdf = new jsPDF({ orientation:'l', unit:'mm', format:[OFICIO_MM.w, OFICIO_MM.h] });
    pdf.addImage(imgData, 'PNG', 0, 0, OFICIO_MM.w, OFICIO_MM.h);
    const safeName = (p.name||'quiniela').toLowerCase().replace(/[^a-z0-9]+/g,'_');
    if(pdfWindow && !pdfWindow.closed){
      pdfWindow.location.href = pdf.output('bloburl');
      toast('PDF generado en una pestaña nueva.');
    }else{
      // El navegador bloqueó la pestaña (o el usuario la cerró): se
      // ofrece la descarga normal como respaldo, para que el PDF nunca
      // se pierda.
      pdf.save(`quiniela_${safeName}.pdf`);
      toast('Tu navegador bloqueó la pestaña nueva — se descargó el PDF en su lugar.');
    }
  }).catch(()=>{
...
// v6.0 (Fase 1) — Conectamos la sincronización Firestore en cuanto el
// módulo de Firebase esté listo. Mismo patrón que usa app.js al final de
// su propio archivo (si window.__fb ya existe, conectamos de inmediato;
// si no, esperamos el evento "firebase-ready" que dispara index.html).
if(window.__fb){
  rgWireFirestoreSync();
}else{
