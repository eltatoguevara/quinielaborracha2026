...
function loadData(){
  try{
    ...
    if(!d.papelera) d.papelera=[]; // v6.1 — participantes eliminados van aquí (recuperables), no se borran de una
...
function renderAdmin(){
  ...
  document.getElementById('a_reset').addEventListener('click', ()=>{
    ...
    DB = {participants:[], predictions:{}, papelera:[], nextSeq:1, configGlobal:{modoConsultaHabilitado:true, registroAbierto:true, loginPorNombreHabilitado:true, fechaCierre:'', horaCierre:'23:59'}};
    ADMIN_SEARCH = '';
    ADMIN_FILTER = 'all';
    SHOW_PAPELERA = false;
    saveData(DB);
